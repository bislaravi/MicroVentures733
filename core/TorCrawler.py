...
class TorCrawler(object):
    ...
    @staticmethod
    def get_headless_browser(url, headers=None):
        """Return either BeautifulSoup object or raw response from GET."""


        firefox_driver = os.path.abspath(os.getcwd()) +"/core/geckodriver"
        os.environ["webdriver.firefox.driver"] = firefox_driver
        firefox_options = webdriver.ChromeOptions()
        # firefox_options.add_argument('--proxy-server=http://127.0.0.1:8118')
